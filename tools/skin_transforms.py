    ...
    try:
        if xbmc.getCondVisibility('System.Platform.TVOS'):
            _km = '\\n'.join([
                ...
                '  </FullscreenVideo>',
                '  <FullscreenLiveTV>',
                '    <!-- select opens the OSD on LIVE TV only (Fire TV parity:',
                '         its select maps to OSD in fullscreen video). Upstream',
                '         maps select to Pause, which live streams without',
                '         timeshift silently swallow - a dead button. Movies and',
                '         shows keep select=Pause via the FullscreenVideo',
                '         fallback. -->',
                '    <customcontroller name="SiriRemote">',
