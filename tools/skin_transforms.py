...
_RESET_MENU_ACTION = (
    ...
    "            out = []\n"
    "            try:\n"
    "                from skinshorcuts import datafunctions as _df\n"
    "                from skinshorcuts import constants as _cn\n"
    "                out.append('DATA_PATH=' + _cn.DATA_PATH)\n"
    "                out.append('MASTER_PATH=' + _cn.MASTER_PATH)\n"
    "                out.append('PROFILE_PATH=' + _cn.PROFILE_PATH)\n"
    "                for _tag, _p in (('data', _cn.DATA_PATH), ('master', _cn.MASTER_PATH)):\n"
    "                    _fp = _p + 'mainmenu.DATA.xml'\n"
    "                    try:\n"
    "                        _t = ET.parse(_fp)\n"
    "                        _dis = any((_s.findtext('defaultID') or '') == 'games' and _s.find('disabled') is not None for _s in _t.getroot().findall('shortcut'))\n"
    "                        out.append('rawfile[%s]=%s games_disabled=%s' % (_tag, _fp.split('addon_data')[-1], _dis))\n"
    "                    except Exception as _e2:\n"
    "                        out.append('rawfile[%s]_err=%s' % (_tag, _e2))\n"
    "                d = _df.DataFunctions()\n"
    "                tree = d.get_shortcuts('mainmenu', profile_dir=xbmcvfs.translatePath('special://masterprofile/'))\n"
    "                for sc in tree.getroot().findall('shortcut'):\n"
    "                    if (sc.findtext('defaultID') or '') == 'games':\n"
    "                        out.append('getshortcuts_games_disabled=%s' % (sc.find('disabled') is not None))\n"
    "                        break\n"
    "            except Exception as e:\n"
    "                out.append('ERR:%s' % e)\n"
    "            xbmcgui.Window(10000).setProperty('t7b_menuprobe', (' | '.join(out))[:1400])\n"
    "        elif sys.argv[1] == 'writeTest':\n"
    "            r = []\n"
    "            newp = 'special://skin/xml/t7b-writetest.txt'\n"
    "            incp = 'special://skin/xml/script-skinshortcuts-includes.xml'\n"
    "            try:\n"
    "                f = xbmcvfs.File(newp, 'w')\n"
    "                f.write('T7B-' + sys.argv[2])\n"
    "                f.close()\n"
    "                f = xbmcvfs.File(newp)\n"
    "                r.append('newfile_read=' + f.read())\n"
    "                f.close()\n"
    "                r.append('newfile_del=%s' % xbmcvfs.delete(newp))\n"
    "            except Exception as e:\n"
    "                r.append('newfile_err=%s' % e)\n"
    "            try:\n"
    "                before = xbmcvfs.Stat(incp).st_size() if xbmcvfs.exists(incp) else -1\n"
    "                r.append('inc_del=%s' % (xbmcvfs.delete(incp) if xbmcvfs.exists(incp) else 'absent'))\n"
    "                r.append('inc_exists_after_del=%s' % xbmcvfs.exists(incp))\n"
    "                r.append('inc_size_before=%s' % before)\n"
    "            except Exception as e:\n"
    "                r.append('inc_err=%s' % e)\n"
    "            xbmcgui.Window(10000).setProperty('t7b_writetest', ' '.join(r))\n"
